...
'''
from ro.roi_align.modules.roi_align import RoIAlignAvg, RoIAlign
from ro.rod_align.modules.rod_align import RoDAlignAvg, RoDAlign
...
class GlobalContentPreserveModule(nn.Module):
    def __init__(self, in_dim, inter_dim, out_dim, mode='conv'):
        super(GlobalContentPreserveModule, self).__init__()
        if mode == 'conv':
    ...
    def forward(self, feat_map, crop_mask, heat_map):
        '''
        ...
        # print('relation_feat', relation_feat.shape) # [1, 8, 16, 22]  [1, 8, 24, 16]
        pre_heat_map = self.preserve_predict(relation_feat)
        # print('heat_map', pre_heat_map.shape) # heat_map torch.Size([1, 1, 64, 64])
        # print('crop_mask', crop_mask.shape) # crop_mask torch.Size([1, 64, 64, 64])
        B, N, H, W = crop_mask.shape
        # B, N, H, W = heat_map.shape
        rep_heat = repeat(pre_heat_map, 'b 1 h w -> b n h w', n=N)
        if heat_map == None:
            real_heat = rep_heat
        else:
            real_heat = repeat(heat_map, 'b 1 h w -> b n h w', n=N)
        cat_map = torch.stack([rep_heat, real_heat], dim=2)
        # print("cat_map", cat_map.shape) # torch.Size([1, 1, 2, 64, 64])
        # print('1', cat_map.shape)
        # cat_map = torch.stack([rep_heat], dim=2)  # 只使用heat_map
        # print('2:', cat_map.shape)
        # cat_map = rep_heat
        cat_map = rearrange(cat_map, 'b n c h w -> (b n) c h w', b=B, n=N)
        # print('cat_map', cat_map.shape) # torch.Size([1, 2, 64, 64])
        content_feat = self.content_feat_layer(cat_map)
        # print('content_feat', content_feat.shape) # [64, 256]
        return pre_heat_map, content_feat
...
class GraphResoningModule(nn.Module):
    ...
    def forward(self, feat_map):
        ...
        return updated
# class GlobalCentricCroppingModel(nn.Module):
#     def __init__(self, loadweights=True, cfg=None):
#         super(GlobalCentricCroppingModel, self).__init__()
#         if cfg is None:
...
#             self.content_preserve = GlobalContentPreserveModule(reduce_dim, 8, cp_dim, mode=cfg.content_preserve_type)
#         else:
...
#         contain_human = (torch.nonzero(human_mask[0, 4]).numel() > 0)
...
class HumanCentricCroppingModel(nn.Module):
    ...
    def forward(self, im, crop_box, human_box, crop_mask, human_mask, real_heat_map):
        B, N, _ = crop_box.shape
        if crop_box.shape[-1] == 4:
            index = torch.arange(B).view(-1, 1).repeat(1, N).reshape(B, N, 1).to(im.device)
            crop_box  = torch.cat((index, crop_box), dim=-1).contiguous()
        ...
        f5 = F.interpolate(f5, size=f4.shape[2:], mode='bilinear', align_corners=True)
        agg_feat = self.f3_layer(f3) + self.f4_layer(f4) + self.f5_layer(f5)
        red_feat = self.feat_ext(agg_feat) # red_feat torch.Size([1, 32, 16, 22])
        # print('red_feat', red_feat.shape)
        # print('heat_map', heat_map.shape) # heat_map torch.Size([1, 1, 64, 64])
        # contain_human = (torch.count_nonzero(human_mask[0,4])
        contain_human = (torch.count_nonzero(human_mask[0,4]) > 0)
        part_feat = torch.zeros_like(human_mask.detach())
        ...
        if self.content_preserve:
            heat_map, cont_feat = self.content_preserve(red_feat, crop_mask, real_heat_map)
            if self.cfg.only_content_preserve:
                crop_feat = cont_feat
            else:
                crop_feat = torch.cat([crop_feat, cont_feat], dim=1)
        ...
        return part_feat, heat_map, score
# 这个网络结构用于cpc
class HumanCentricCroppingModel_CPC(nn.Module):
    def __init__(self, loadweights=True, cfg=None):
        super(HumanCentricCroppingModel_CPC, self).__init__()
        if cfg is None:
